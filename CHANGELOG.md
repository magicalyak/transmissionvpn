...
- **`vpn-monitor` never stopped Transmission.** Two bugs, each enough on its own. `stop_transmission` checked `pgrep transmission-daemon`, but `pgrep` matches the kernel process name, which is cut to 15 characters (`transmission-da`), so it never matched and the function did nothing; procps even warns about this. Behind that check, `s6-svc` was pointed at `/var/run/s6/services/transmission`, the s6-overlay v2 path, while the image runs s6-overlay 3.2.1.0 with the service at `/run/service/svc-transmission`. The monitor now matches `transmission-da` exactly, takes the v3 service down and waits for it, and falls back to killing the daemon. `restart_transmission` and the monitor's `finish` script had the same problems.
- **Transmission started before the kill switch existed.** s6-overlay starts services in parallel with the cont-init scripts, so Transmission, cron and Privoxy ran on the container's empty, all-ACCEPT firewall until `vpn-setup.sh` (cont-init 50) locked it down. In a test pod Transmission was answering on 9091 about a second before that. With existing torrents in `/config`, that is enough to announce from the host's real address. `init-services`, `svc-transmission` and `privoxy` now depend on `legacy-cont-init`, so they start only after the tunnel and kill switch are up.
- **The web UI download ran before the VPN.** With `TRANSMISSION_WEB_UI_AUTO` set, cont-init 04 fetched the UI from GitHub before `vpn-setup.sh` ran. It is now cont-init 60, after the tunnel is up, and skips the download if VPN setup did not complete. A UI already in `/config/web-ui` is still reused without a download.
...
- **Removed the temporary eth0 DNS rules from the kill switch.** `vpn-setup.sh` added `temp-vpn-dns` ACCEPT rules for port 53 on eth0 and deleted them again a few lines later, and `vpn-monitor`'s restart path added a `temp-restart-dns` rule that stayed until the next flush. They were meant to let the VPN server hostname resolve, but `vpn-setup.sh` added them after the lookup, so they never did anything. Remote hostnames now resolve through the tunnel, which is up and allowed by then, so the kill switch never opens DNS off the VPN.
...
**Upgrade note.** Two behaviours change on containers that do not override them. `transmissionvpn_vpn_connected` now means traffic passes through the tunnel, so alerts on it and on `transmissionvpn_healthy` will fire for a tunnel that is up but dead, which they never did before. Both also read `0` for the first check after startup (roughly `VPN_INITIAL_DELAY` plus one probe) until `vpn-monitor` has verified the tunnel. And with `AUTO_RESTART_VPN=true`, the container now exits with code 1 once `MAX_RESTART_ATTEMPTS` is used up, instead of idling. Run it with a restart policy (`restart: unless-stopped`; Kubernetes restarts it by default), or set `EXIT_ON_MAX_RESTARTS=false` to keep the old behaviour.

### Fixed
- **`transmissionvpn_vpn_connected` reported an address, not a connection.** The metrics server set `connected` as soon as a tun/wg/tap interface had an IPv4 address. During the outage tun0 held `10.25.18.69` throughout while ping and HTTPS through it both failed, so the gauge read `1` for 38 hours, and `transmissionvpn_healthy` read `1` with it. `connected` now comes from `vpn-monitor`'s probe through the tunnel, which it publishes to `/tmp/vpn_tunnel_state` after every check, in the same way `pia-pf-firewall.sh` publishes rule state for the unprivileged metrics server. Scrapes only read that file, so a probe can never block `/metrics`. A result older than `max(3 x VPN_CHECK_INTERVAL, 120s)` counts as not connected, so a stuck or dead monitor cannot freeze the last good answer in place. `transmissionvpn_vpn_interface_up` keeps its meaning, so "no tunnel" (`interface_up 0`) and "tunnel up but dead" (`interface_up 1`, `connected 0`) are now distinguishable.
- **`vpn-monitor` gave up permanently after `MAX_RESTART_ATTEMPTS`.** It logged `Maximum restart attempts (3) exceeded - manual intervention required` every ~35 seconds indefinitely. Exiting the service would not have helped, since s6 restarts it into the same counter file and the same dead tunnel, so it now stops the whole container: it writes the exit code to `/run/s6-linux-init-container-results/exitcode` and calls `/run/s6/basedir/bin/halt`. Both were verified against `4.1.3-r0-ls362` (s6-overlay 3.2.1.0), and a built image with a dummy `tun0` that has an address but carries nothing exits with code 1. This is controlled by the new `EXIT_ON_MAX_RESTARTS` (default `true`). The restart counter lives in `/tmp`, which survives `docker restart`, so a new `cont-init` step clears it once per container start; otherwise a restarted container would come back already at the limit and exit again without trying. The counter itself was already scoped to sustained health (it resets after 5 consecutive healthy checks, about 2.5 minutes at the default interval). The `restarts: 2/3` seen during the outage came from a tunnel flapping faster than that, which is exactly when escalating to a container restart is the right call.
- **A VPN restart counted as successful if the interface came back with an address.** That is the same test that misled the metric, and it would have reset the failure count on a restart into a tunnel that still carried nothing. A restart now has to pass the tunnel probe.
- **`vpn-monitor` used a different, weaker connectivity test from the health check.** It sent a single ping to a hardcoded `1.1.1.1`, while `healthcheck.sh` sends three packets and falls back to `HEALTH_CHECK_HOST_FALLBACK`, the hardening added in v4.1.2-r2 against ICMP rate-limiting. One dropped reply was enough to count a failure and reset the healthy streak that clears the restart counter. Both now source one probe, `root/vpn-probe.sh` (`/usr/local/bin/vpn-probe.sh` in the image), which also carries the private-`HEALTH_CHECK_HOST` override so `vpn-monitor` honours it too.
- **The PIA port-forward keepalive was never restarted.** Its loop exits on the first failed bind, which is guaranteed once the tunnel has been down for 15 minutes, and nothing restarted it. The oneshot that starts it runs only when the container starts, and an in-place VPN restart bypasses it. `vpn-monitor` now re-runs `pia-port-forward.sh` after every successful VPN restart, since the old keepalive is bound to the previous session's gateway and signature. It also re-runs it whenever the tunnel is healthy but the keepalive is not running, at most once per `PF_RESTART_COOLDOWN_SECONDS` (default 900) and never while a setup run is in progress.
- **With PIA port forwarding on and the tunnel down, a closed port was blamed on PIA.** `classify_port_state` reported `pf_port_bound_but_unreachable` ("the binding is gone upstream") whenever the port tested closed with the firewall rules present. Now that `vpn_connected` can actually be false, that would have pointed at the wrong fault during every outage. It is now reported only with the tunnel up; `vpn_disconnected` already covers the real cause. `pf_rules_missing` is unaffected.

### Added
- **`transmissionvpn_vpn_tunnel_check_age_seconds`** (age of the last probe, `-1` before the first) and **`transmissionvpn_vpn_restart_attempts`** (automatic restarts since the tunnel was last stable). `/health` gains `vpn.tunnel_check` with the same data.
- **`EXIT_ON_MAX_RESTARTS`** (default `true`) and **`PF_RESTART_COOLDOWN_SECONDS`** (default `900`), documented in README with the existing restart settings, which were missing from the table.
- **`test-tunnel-metrics.py`**: 20 assertions that `vpn_connected` and `healthy` read `0` for an interface that is up with an address but fails the probe, including stale, missing and interval-scaled results and the port-forward classification with the tunnel down.
- **`test-vpn-monitor-recovery.sh`**: 26 assertions over the monitor's tunnel check, the container exit at `MAX_RESTART_ATTEMPTS` (exit code written, halt called, never returning to the loop), `EXIT_ON_MAX_RESTARTS=false`, a restart into a dead tunnel, and port-forward recovery. Like the other monitor tests it extracts functions from the shipped scripts and stubs `ip`, `ping` and `iptables`. It needs bash 4, so on macOS run it with `docker run --rm -v "$PWD":/src -w /src bash:5 bash test-vpn-monitor-recovery.sh`. Both new tests were mutation-checked against the original behaviour.
- **`docs/TROUBLESHOOTING.md`**: what the new exit means, and how to reproduce a dead tunnel locally, either with a DROP rule on a real tunnel or with a dummy interface and no VPN at all.
